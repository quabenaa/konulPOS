...
<?php
  ...
  echo "<h2><center>WASTAGE REPORT</center></h2> ";

if (empty($filter))
{
  $result = mysql_query ("SELECT `ID`,`Stock Code`,`Stock Name`,`Wastage Date`,`Qnty`,`Detected By`,`Approved By` FROM `wastage` order by `Wastage Date` Desc"); 
} else {
  $result = mysql_query ("SELECT `ID`,`Stock Code`,`Stock Name`,`Wastage Date`,`Qnty`,`Detected By`,`Approved By` FROM `wastage` order by `Wastage Date` Desc"); 
}
  ...
  <TH bgcolor='#C0C0C0' width='15%'> <font style='font-size: 9pt'>WASTAGE DATE </font>&nbsp;</TH>
  <TH bgcolor='#C0C0C0' width='15%'><font style='font-size: 9pt'>PRODUCT NAME</font>&nbsp;</TH>
  <TH bgcolor='#C0C0C0' width='15%'><font style='font-size: 9pt'>WASTAGE QUANTITY</font>&nbsp;</TH>
  <TH bgcolor='#C0C0C0' width='15%'><font style='font-size: 9pt'>DETECTED BY</font>&nbsp;</TH>
  <TH bgcolor='#C0C0C0' width='25%'><font style='font-size: 9pt'>APPROVED BY</font>&nbsp;</TH>
  </TR>";

	$n=0;
	while(list($id,$name,$code,$dates,$given,$reqby,$givenby)=mysql_fetch_row($result))
	{
		$n++;
      echo "<TR>
