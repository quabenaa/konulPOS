 ...
 switch ($_POST['submit'])
   {
      ...
      if (trim($cod) != "")
      {
        ...
        $result_update = mysql_query($query_update);
            ...
            $query_update_Log = "Insert into `Monitor` (`User Category`, `User Name`,`Date Logged on`, `Time Logged on`,`File Used`,`Details`) 
                  VALUES ('" . $rows['access_name'] . "','" . ucfirst($_SESSION['name']) . "', '" . date('Y/m/d') . "', '" . date('h:i A') . "','Returns Record','Modified Return Sales Record for Stock: " . $cod . ", " . $stockname . "')";

            $result_update_Log = mysql_query($query_update_Log);
            ##################### END MONITOR ##################### 

        $tval="Your record has been updated.";
        header("location:returns.php?cod=$cod&tval=$tval&redirect=$redirect");
      }
      else
      {
        $tval="Please enter Stock Code before updating.";
        header("location:returns.php?id=$id&cod=$cod&tval=$tval&redirect=$redirect");
      }
      break;
	 ...
	 ################### CUSTOMER ENQURY ###################  
	   if ($paid=='credit')
      {
		  if(empty($soldto)){
			...
			header("Location:returns.php?cVal=$val");
			die(); 
		  }
	  ...
	  mysql_query("DELETE FROM `sales` WHERE `cust_id` = '" . $_SESSION['cust_return'] . "'") or die(mysql_error());
	   
	  mysql_query("UPDATE `tempsales`SET `Paid` = '$paid Sales' WHERE `cust_id` = '" . $_SESSION['cust_return'] . "'") or die(mysql_error());
      ...
      { 
       ...
       while(list($stcod,$stname,$stdate,$abal,$stto,$buyn,$cid)=mysql_fetch_row($result_csh))
       {
		...
		$url="returnreciept.php?credit=$soldto";
	...
	################### IF CASH SALES ###################
	  ...
	  $query_csh = "select `Stock Code`,`Stock Name`,`Sales Date`,`Balance`,`Sold To`,`Sold Name`,`cust_id`  from `tempsales`
	   ...
	   $url="returnreciept.php?cash=$deposit&change=$total&redirect=$redirect";
      } 
	 ################### END CASH SALES ###################
	...
	############################################# DELETE SALES ITEM ################################################################
     ...
     case 'Cancel':
      {
       $query_delete = "DELETE FROM `tempsales` WHERE `cust_id` = '" . $_SESSION['cust_return'] . "'";
       ...
       $result_delete = mysql_query($query_delete);          
        ...
        $tval="Your Return Sales has been Cancelled.";
        header("location:return.php?cod=$cod&tval=$tval&redirect=$redirect");
      }
      break;       
   }
 }
...
case 'Save Only':
      ...
      { 
        ...
        header("location:returns.php?code=$cod&tval=$tval&redirect=$redirect");
      }
      else
      {
        $tval="Please enter Stock Code before saving.";
        header("location:returns.php?id=$id&code=$cod&tval=$tval&redirect=$redirect");
      }
      break;
*/