...
echo "<tr><td colspan=4 align='left'><font color='#000000' style='font-size: 11pt'>
 <hr/>
  ...
  $query="SELECT `ID`,`Stock Name`,`Qnty Sold`,`Unit Cost` FROM sales WHERE `Entered By` ='" . strtoupper($_SESSION['name']) . "' and `Sales Date`='" . date('Y-m-d') . "' and `cust_id` = '" . $_SESSION['cust_token'] . "' order by `ID` desc";
  $result=mysql_query($query);

  while(list($id,$name,$qnty,$price)=mysql_fetch_row($result))
  {
...
//$cash = number_format($cash_, 2);
 ...
 Cashier : " . strtoupper($_SESSION['name']) ."
  </font></td></tr>";		
...
window.location = "sales.php?tval=$tval";
</script>