...
@$filename = "debtorsreport_" . date('Ymd') . $filter . ".xls";
...
<?php
  ...
  echo "<h2><center>DEBTORS REPORT</center></h2> ";

if (empty($filter))
{
  $query="SELECT `ID`,`Date`,Company,`Contact Person`,Amount,`Paid`,`Balance` FROM debtor order by `Date` desc";
} else {
  $query="SELECT `ID`,`Date`,Company,`Contact Person`,Amount,`Paid`,`Balance` FROM debtor order by `Date` desc";
}
$result = mysql_query($query) or die(mysql_error());
 ...
 $sql_tt = "SELECT SUM(Amount) as amt, SUM(`Paid`) as paid, SUM(`Balance`) as bal FROM debtor"; 
