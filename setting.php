			...
			<div id="main-content" class="clearfix">
					...
					<div id="page-content" class="clearfix">
                       ...
                       <?php if(isset($_REQUEST['bank']) ){
						echo '<form action="submitbank.php" method="post">'; 
					   }else if(isset($_REQUEST['status']) ){
					   	echo '<form action="submitstatus.php" method="post">';
					   }else if(isset($_REQUEST['cate'])){
					   echo '<form action="submitscat.php" method="post">';
					   } ?>                        
							...
							<div class="row-fluid">
								...
								if(isset($_REQUEST['bank']) ){
									...
									$ID=$_REQUEST['ID'];									
									$sql="SELECT `BankID`,`Name` FROM `bank` WHERE `BankID`='$ID'";
									$result = mysql_query($sql) or die('Could not look up user data; ' . mysql_error());
									$row = mysql_fetch_array($result);
									}
								?>
								   <tr>
								    <td class="controls controls-row"><label class="small-labels">Bank Name</label>
									<span class="input-icon"><i class="icon-leaf"></i>
                                    <input type="text" class="input-large"  name="bank" id="bank" size="31" value="<?php echo @$row['Name']; ?>">
                                    </span>
        							<input type="hidden" name="id" size="31" value="<?php echo @$ID; ?>">
                                    </td>
						          </tr>
                                 <?php
								}else if(isset($_REQUEST['status']) ){
									if(isset($_REQUEST['ID'])){
									$ID=$_REQUEST['ID'];	
									$sql="SELECT `id`,`Status` FROM `status` WHERE `id`='$ID'";
									$result = mysql_query($sql,$conn) or die('Could not look up user data; ' . mysql_error());
									$row = mysql_fetch_array($result);
									}
								?>
                                 <tr>
								    <td class="controls controls-row"><label class="small-labels">Status</label>
									<span class="input-icon"><i class="icon-circle"></i>
                                    <input type="text" name="code" id="code" class="input-large" value="<?php echo @$row['Status']; ?>">
                                 ...
                                 <?php
									...
									if(isset($_REQUEST['ID'])){
									$ID=$_REQUEST['ID'];
									$sql="SELECT `ID`,`Category` FROM `stock category` WHERE `ID`='$ID'";
									$result = mysql_query($sql,$conn) or die('Could not look up user data; ' . mysql_error());
									$row = mysql_fetch_array($result);
									}
								?> <tr>
									...
									<span class="input-icon"><i class="icon-circle"></i>
                                    <input type="text" name="category" id="category" size="31" value="<?php echo @$row['Category']; ?>">
                                    </span>
                                    <input type="hidden" name="catid"  size="20" value="<?php echo @$row['ID']; ?>"></td>
						          </tr>
                                 <?php
